...
class GradientFields(object):
    ...
    @staticmethod
    def spatial_gradient_integral_on_cad(sim, forward_fields, adjoint_fields, wl_scaling_factor, num_params, obj_bounds = False):
        """ Computes the spatial gradient integral in FDTD for the figure of merit derivatives.
        
        Parameters
        ----------
        sim: The simulation object used to execute Lumerical FDTD commands.
        forward_fields: Name of the forward fields dataset.
        adjoint_fields: Name of the adjoint fields dataset.
        wl_scaling_factor: Scaling factor for each wavelength point.
        num_params: Number of optimization parameters.
        obj_bounds: If True, restricts integration to specific bounds. Defaults to False.

        Notes
        -----
        - num_params is passed explicitly since evaluating size(d_eps) unavoidably creates a copy of d_eps.
        - If `obj_bounds` is True, the method adjusts the integration bounds based on precomputed indices 
          (`x_min_indices`, `x_max_indices`, `y_min_indices`, `y_max_indices`).
        - A single fdtd.eval() call is used to minimize overhead from repeated API calls.
        """
        ...
        sim.fdtd.eval("gradient_fields = 2.0 * eps0 * {0}.E.E * {1}.E.E;".format(forward_fields, adjoint_fields) +
                      "num_opt_params = {0};".format(num_params) +
                      "num_wl_pts = length({0}.E.lambda);".format(forward_fields) +
                      "partial_fom_derivs_vs_lambda = matrix(num_wl_pts, num_opt_params);" +
                      "if (exist('x_min_indices')) { " +
                            "x = zeros(length(forward_fields.E.x)+2);"+
                            "y = zeros(length(forward_fields.E.y)+2);"+
                            "x(2:length(x)-1) = forward_fields.E.x;"+
                            "y(2:length(y)-1) = forward_fields.E.y;"+
                            "x(1) = 2*x(2) - x(3); " +
                            ...
                            "        spatial_integrand = zeros(x_max_indices(param_idx)-x_min_indices(param_idx)+3, y_max_indices(param_idx)-y_min_indices(param_idx)+3, size(d_epses{param_idx},3));"+
                            "        spatial_integrand(2:x_max_indices(param_idx)-x_min_indices(param_idx)+2,2:y_max_indices(param_idx)-y_min_indices(param_idx)+2,:) = pinch(sum(gradient_fields(x_min_indices(param_idx):x_max_indices(param_idx),y_min_indices(param_idx):y_max_indices(param_idx),:,wl_idx,:) * wl_scaling_factor(wl_idx) * d_epses{param_idx}, 5), 4);"+
                            "        partial_fom_derivs_vs_lambda(wl_idx, param_idx) = integrate2(spatial_integrand, [1,2,3], x(x_min_indices(param_idx):x_max_indices(param_idx)+2), y(y_min_indices(param_idx):y_max_indices(param_idx)+2), forward_fields.E.z);"+
                                "}"+
                            "}"+
                      "} else {" +
                      "for(param_idx = [1:num_opt_params]){"+
